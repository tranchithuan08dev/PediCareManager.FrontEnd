...
const PatientExaminationFormBK_v1 = () => {
    ...
    // ✅ SỬA: Tự động điền cân nặng/chiều cao gần nhất khi load history
    useEffect(() => {
        if (patientCodeKeyword) {
            setMedicalHistory(history);

            if (history.length > 0) {
                // Sort để lấy gần nhất trước
                const sorted = [...history].sort((a, b) => new Date(b.visitDate) - new Date(a.visitDate));

                // Lấy cân nặng gần nhất có giá trị
                const latestWithWeight = sorted.find(h => h.weight_kg != null && h.weight_kg > 0);
                // Lấy chiều cao gần nhất có giá trị
                const latestWithHeight = sorted.find(h => h.height_cm != null && h.height_cm > 0);

                form.setFieldsValue({
                    weightKg: latestWithWeight ? latestWithWeight.weight_kg : undefined,
                    heightCm: latestWithHeight ? latestWithHeight.height_cm : undefined,
                });
            }
        }
    ...
    const handleLoadLastPrescription = () => {
        ...
        // Sort để chắc chắn lấy gần nhất
        ...
        // Map prescriptionItems từ history sang format hiện tại
        const newItems = latest.prescriptionItems.map((item, idx) => {
            ...
            return {
                ...
                totalPrice: item.quantity * (matchedMedicine?.priceSell || 0),
                morning: 0,
                noon: 0,
                afternoon: 0,
                evening: 0,
        ...
        messageApi.success(
            `Đã lấy toa ngày ${moment(latest.visitDate).format('HH:mm DD/MM/YYYY')} — ${newItems.length} thuốc.`
        );
    };
            ...
            <Content style={{ padding: '10px 24px' }}>
                <div style={{ padding: '10px', background: '#fff' }}>
                    ...
                    <Card title={<><MedicineBoxOutlined /> Kê Đơn Thuốc Hiện Tại</>} size="small" bordered={false}>
                        ...
                        {/* ✅ MỚI: Row chứa Số ngày uống + nút Lấy toa gần nhất */}
                        {/* Row compact: Số ngày + Lấy toa gần nhất cùng hàng */}
...
export default PatientExaminationFormBK_v1;